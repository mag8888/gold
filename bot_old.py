...
async def get_referral_link(message: types.Message):
    ...
    await message.answer(
        f"🔗 **Ваша реферальная ссылка:**\n[Нажмите для копирования]({referral_link})\n\n"
...
🔗 Ваша реферальная ссылка: [Нажмите для копирования]({referral_link})
...
async def process_habit_frequency_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.data == "freq_custom":
        await callback.message.edit_text(
            "🔧 Введите желаемую частоту числом (сколько раз в день/неделю):",
            reply_markup=None
        )
        await callback.answer()
        return
...
async def start_habit_reminders(user_id: int, habit_id: int):
    """Запуск напоминаний для привычки каждые 5 минут"""
    ...
    task = asyncio.create_task(habit_reminder_loop(user_id, habit_id))
...
async def habit_reminder_loop(user_id: int, habit_id: int):
    """Цикл отправки напоминаний каждые 5 минут"""
    try:
        while True:
            ...
            reminder_text = f"⏰ **Напоминание о привычке**\n\n" \
                          f"📅 {habit_name}\n" \
                          f"🎯 Цель: {target_frequency} раз сегодня\n" \
                          f"✅ Выполнено: {completed_today} раз\n\n" \
                          f"Не забудьте выполнить привычку!"
            ...
            try:
                ...
                logger.info(f"Sent reminder for user {user_id}, habit {habit_id}")
            except Exception as e:
                logger.error(f"Failed to send reminder: {e}")
            
            # Ждем 5 минут до следующего напоминания
...
async def test_reminders_command(message: types.Message):
    ...
    await start_habit_reminders(user_id, habit_id)
    
    await message.answer(
        f"🔔 **Тестирование напоминаний запущено!**\n\n"
        f"📅 Привычка: {habit_name}\n"
        f"⏰ Напоминания каждые 5 минут\n"
